	...
	<div class="layout">
		...
		<div class="content">
			<div class="header-with-button">
				<h1 id="linux-dhcp">Linux – DHCP na Debianu 12</h1>
				<a href="/All_Matriculation_Subjects/Operating_system/LinuxOp_20_1_DHCP/Exam-LinuxOp_20_1_DHCP" class="ButtonExam">Napsat si test</a>
			</div>

			<section id="uvod" class="note">
				<h2>📘 Úvod do DHCP v Linuxu</h2>
				<p>
					DHCP (Dynamic Host Configuration Protocol) slouží k automatickému přidělování IP adres v
					síti. V Debianu 12 (Bookworm) se často využívá balíček <code>isc-dhcp-server</code>.
				</p>
				<ul>
					<li>Zjednodušuje správu IP adres</li>
					<li>Automatizuje konfiguraci síťových zařízení</li>
				</ul>
			</section>

			<section id="predpoklady" class="note">
				<h2>🛠️ Požadavky</h2>
				<ul>
					<li>Operační systém: Debian 12</li>
					<li>Uživatel s oprávněními root nebo sudo</li>
					<li>Základní znalost sítě a IP adresování</li>
				</ul>
			</section>

			<section id="instalace" class="note">
				<h2>📦 Instalace DHCP serveru</h2>
				<pre><code
						>sudo apt update
sudo apt install isc-dhcp-server</code
					></pre>
			</section>

			<section id="konfigurace" class="note">
				<h2>⚙️ Konfigurace souboru dhcpd.conf</h2>
				<p>Soubor: <code>/etc/dhcp/dhcpd.conf</code></p>
				<ul>
					<li>Nastav rozsah adres (<code>subnet</code>) a gateway (<code>option routers</code>)</li>
					<li>Nastav DNS servery pomocí <code>option domain-name-servers</code></li>
				</ul>
				<pre><code>sudo nano /etc/dhcp/dhcpd.conf</code></pre>
			</section>

			<section id="rozhrani" class="note">
				<h2>🌐 Nastavení síťového rozhraní</h2>
				<p>Soubor: <code>/etc/default/isc-dhcp-server</code></p>
				<p>Zadej název rozhraní, např. <code>eth0</code></p>
				<pre><code>sudo nano /etc/default/isc-dhcp-server</code></pre>
				<p>Poté restartuj systém:</p>
				<pre><code>sudo reboot</code></pre>
			</section>

			<section id="sluzba" class="note">
				<h2>🚀 Spuštění a povolení služby DHCP</h2>
				<pre><code
						>sudo systemctl start isc-dhcp-server
sudo systemctl enable isc-dhcp-server</code
					></pre>
				<p>Kontrola stavu:</p>
				<pre><code>sudo systemctl status isc-dhcp-server</code></pre>
			</section>

			<section id="firewall" class="note">
				<h2>🛡️ Konfigurace firewallu</h2>
				<h3>UFW:</h3>
				<pre><code>sudo ufw allow 67/udp</code></pre>
			</section>

			<section id="ukoly" class="note">
				<h2>📋 Úkoly k procvičení</h2>
				<ol>
					<li>Jak nastavit statickou IPv4 adresu na Debian 12?</li>
					<li>Jaký balíček je třeba pro běh DHCP serveru?</li>
					<li>Jak ověřit, že port 67 je otevřený?</li>
					<li>Jak povolit port 67 ve firewallu (UFW i iptables)?</li>
					<li>Jak ověřit, že je port 67 opravdu povolen?</li>
				</ol>
			</section>
			<a href="/PDF/LinuxOp_20_1_DHCP.pdf" class="ButtonExam" download>📄 Stáhnout PDF</a>
		</div>
	</div>
</main>
